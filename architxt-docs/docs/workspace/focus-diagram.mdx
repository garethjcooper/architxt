...
The Preview tab can render Mermaid flowcharts with the default `dagre` renderer or the `elk` renderer. The `elk` renderer is useful for complex diagrams with many crossing edges, because it produces a tidier layout. You can switch renderers in the diagram controls without changing the source.
