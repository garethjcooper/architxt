...
Before a mental model is executed or pushed to Hindsight, its **Source Query** is composed. The **Source Query** is written in [AQL](./aql): the architxt Query Language. AQL lets a mental model request specific output shapes — graph, table, diagram, or narrative — using block directives, references, and plain intent text.
...
For details on AQL syntax and directives, see the [AQL page](./aql).
