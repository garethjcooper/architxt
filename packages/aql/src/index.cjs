...
const BLOCK_DIRECTIVES = Object.freeze([
  'graph',
  'table',
  'diagram',
  'narrative',
 ...
 */
function toSectionFocus(aqlQuery) {
...
function formatEntityToken(label, id, type) {
  const qualified = type && !id.startsWith(`${type}:`) ? `${type}:${id}` : id;
  return `[[${label} (${qualified})]]`;
}

function formatEdgeToken(source, target, label) {
