...
test('resolveRestrictions', async (t) => {
  await t.test('auto bank without deploy restriction deploys nothing by default', () => {
  ...
  await t.test('auto bank with explicit allowed_model_types deploys only those types', () => {
    const result = resolveRestrictions({
      mode: 'auto',
  ...
  await t.test('manual bank with explicit allowed_model_types deploys only those types', () => {
    const result = resolveRestrictions({
      mode: 'manual',
      restriction: { deploy: { allowed_model_types: ['entity-summary'] } },
    });
    assert.deepEqual(result.deploy.allowed_model_types, ['entity-summary']);
  });

  await t.test('import restrictions merge over defaults', () => {
    const result = resolveRestrictions({
      mode: 'auto',
