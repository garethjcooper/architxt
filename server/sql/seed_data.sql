...
INSERT OR IGNORE INTO mental_models (mm_ext_id, mm_name, mm_source_query, mm_dimension, mm_concatenation, mm_returns, mm_is_template) VALUES
  ('architxt-capabilities-txt-{entity-id}', 'architxt-capabilities-txt-{entity-id}-{entity-name}', 'What are the major architectural capabilities for {entity-name} ({entity-type}:{entity-id})? List their responsibilities, purpose and business capability mapping.', 'capability', 'compile', 'narrative', 'true'),
  ('architxt-interface-found-{entity-id}', 'architxt-interface-found-{entity-id}-{entity-name}', 'What are the flows (apis, data, files and interface calls) into and out of {entity-name} ({entity-type}:{entity-id})? For found flows, describe them contextually. Return each contextual flow as a separate row.', 'interface-found', 'merge', 'graph-discovery', 'true'),
  ('architxt-interface-json-{entity-id}', 'architxt-interface-json-{entity-id}-{entity-name}', 'What are the flows (apis, data, files and interface calls) into and out of {entity-name} ({entity-type}:{entity-id})? For known flows, describe them contextually. Return each contextual flow as a separate row.', 'interface', 'compile', 'graph-known', 'true'),
  ('architxt-summary-txt-{entity-id}', 'architxt-summary-txt-{entity-id}-{entity-name}', 'Describe the core role that {entity-name} ({entity-type}:{entity-id}) plays in the architecture. Return a short summary and a table listing aspect and description.', 'summary', 'compile', 'narrative', 'true');
