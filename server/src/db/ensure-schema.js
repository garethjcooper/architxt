...
const CONTEXTUAL_GRAPH_TEMPLATES = [
  {
    ...
    sourceQuery: 'Entity: {id} ({entity-name}). Describe its core architectural role, responsibilities, and relationships to other components.',
    maxTokens: 8192,
    refreshMode: 'full',
  ...
  },
  {
    ...
    sourceQuery: 'Entity: {id} ({entity-name}). List its major capabilities, each with its purpose, responsibility, business capability mapping, and evidence.',
    maxTokens: 8192,
    refreshMode: 'full',
    refreshAfterConsolidation: 'false',
    excludeAllMentalModels: 'true',
  ...
  },
  {
    ...
    sourceQuery: 'Seed entity: {seed-id} ({seed-name}). Suggest candidate related components, systems, or entities that interact with or are adjacent to this seed, and describe how they connect. Candidates should use the same node id as the seed for any known neighbor; only use a bare lowercase slug for genuinely new candidates.',
    maxTokens: 8192,
    refreshMode: 'full',
    refreshAfterConsolidation: 'false',
    excludeAllMentalModels: 'true',
