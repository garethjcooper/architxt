...
- `name`: the table identifier, used as the property key when applied to a node. If a name is explicitly provided via `#table-name`, use it as the table's `name` field. If no name is provided, generate a short descriptive name (4–6 words) based on the table's content.
