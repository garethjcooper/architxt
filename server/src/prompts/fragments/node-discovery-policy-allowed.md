...
2. If it is **not** listed in the entity catalog above, emit it as a discovered node with a `found:{slug}` id.

Only emit a `found:{slug}` node when all of the following are true:
...
When a discovered element interacts with a known entity, use the known entity's canonical id from the entity catalog above as the edge endpoint. Do **not** invent a new `found:{slug}` alias for a known entity.
