...
export interface NarrativeBlock {
  ...
  /** Stable UUID for the narrative section this block belongs to. */
  __narrativeId?: string;
  /** Stable UUID for the table section this block belongs to (only for synthetic table blocks). */
  __tableId?: string;
  /** Stable UUID for the diagram section this block belongs to (only for synthetic diagram blocks). */
  __diagramId?: string;
}
...
export function buildNarrativeBlocks(narrative: UnifiedNarrativeBlock, idx: number): NarrativeBlock[] {
  ...
  // Inject the array index and stable UUID into every block from this narrative so
  // heading, body, and nested content all map back to the same canonical narrative
  // for delete/restore/edit/evidence actions.
  for (const b of blocks) {
    b.__narrativeIdx = idx;
    b.__narrativeId = narrative.id;
  }
  return blocks;
}
