...
/** Remove a leading `## name` heading that was generated by buildEnvelopeMarkdown for
 *  a named narrative, so the stored narrative body does not duplicate the heading. */
function stripRenderedHeading(markdown: string, heading: string): string {
  if (!heading) return markdown;
  const trimmed = markdown.trimStart();
  const prefix = `## ${heading}`;
  if (!trimmed.startsWith(prefix)) return markdown;
...
export function CuratedPageEditor({
  ...
  const workingEnvelope = useMemo(() => {
    let userMarkdown = buildUserNarrativeContent(displayedBlocks);
    const existingNarratives = envelope.narratives ?? [];
    const firstName = existingNarratives[0]?.narrative_name ?? '';
    // buildEnvelopeMarkdown prepends a ## name heading for named narratives; strip it
    // before storing the body back so the heading is not duplicated on each render.
    if (firstName) {
      userMarkdown = stripRenderedHeading(userMarkdown, firstName);
    }
