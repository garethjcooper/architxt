...
export function EdgeTable({
  ...
  const labelFor = (id: string) => nodeMap.get(id)?.name || nodeMap.get(id)?.label || id;
      ...
      {edges.map((e) => {
        ...
        return (
          <button
            key={key}
            type="button"
            onDoubleClick={() => onInsertToken(formatEdgeToken(e.from, e.to, e.label || e.type || 'edge'))}
            ...
            style={{ borderLeftColor: colorForType(e.type || undefined), borderLeftWidth: 3 }}
          >
            <div className="min-w-0 flex-1 flex flex-col gap-0.5">
              <div className="flex items-center gap-1.5 text-xs text-white/90 truncate">
                <span className="text-white/90 truncate">{labelFor(e.from)}</span>
                ...
                <span className="text-emerald-300/80 truncate">{e.label || e.type || 'edge'}</span>
                <ArrowRight className="w-3 h-3 text-white/40 shrink-0" />
                <span className="text-white/90 truncate">{labelFor(e.to)}</span>
              </div>
