  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <Button onClick={fetchTags} title="Refresh" className="inline-flex items-center justify-center h-8 w-8 rounded text-sm font-medium bg-surface-card border border-border-default text-foreground-default hover:bg-surface-hover transition-colors"><RefreshCw className="h-3.5 w-3.5" /></Button>
        <Button
        ...
        <table className="w-full caption-bottom text-sm">
          <thead>
            <tr className="border-b border-border-default">
              ...
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Tag ID</th>
              ...
              displayTags.map((tag) => (
                <tr
                  ...
                  className={`border-b border-border-subtle transition-colors cursor-pointer ${
                    selected.has(tag.id) ? 'bg-accent-primary-bg' : 'hover:bg-surface-card'
                  }`}
                  onClick={(e) => handleTagClick(tag, e)}
                >
                  <td className={["py-1.5 px-4", freeze && "sticky left-0 z-10 border-r border-border-subtle", selected.has(tag.id) ? "bg-accent-primary-bg" : "bg-surface-card"].filter(Boolean).join(" ")} onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      checked={selected.has(tag.id)}
                      onCheckedChange={() => toggleSelection(tag.id)}
                    />
                  </td>
                  <td className="py-1.5 px-4 text-xs text-foreground-subtle font-mono">{tag.id}</td>
                  ...
                  <td className="py-1.5 px-4 text-xs text-foreground-faint">{tag.generated_by}</td>
