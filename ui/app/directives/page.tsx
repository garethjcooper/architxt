  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <Button onClick={fetchDirectives} title="Refresh" className="inline-flex items-center justify-center h-8 w-8 rounded text-sm font-medium bg-surface-card border border-white/10 text-white hover:bg-surface-hover transition-colors"><RefreshCw className="h-3.5 w-3.5" /></Button>
        <Button
        ...
        <table className="w-full caption-bottom text-sm">
          <thead>
            <tr className="border-b border-white/10">
              ...
              <th className={["text-xs uppercase text-white/60 font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>ID</th>
              ...
              <th className={["text-xs uppercase text-white/60 font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Statement</th>
              <th className={["text-xs uppercase text-white/60 font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Active</th>
              <th className={["text-xs uppercase text-white/60 font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Priority</th>
              <th className={["text-xs uppercase text-white/60 font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Tags</th>
              <th className={["text-xs uppercase text-white/60 font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Generated By</th>
              ...
              displayDirectives.map((directive) => (
                <tr
                  ...
                  className={`border-b border-white/5 transition-colors cursor-pointer ${
                    selected.has(directive.id) ? 'bg-accent-primary-bg' : 'hover:bg-white/5'
                  }`}
                  onClick={(e) => handleDirectiveClick(directive, e)}
                >
                  <td className={["py-1.5 px-4", freeze && "sticky left-0 z-10 border-r border-white/5", selected.has(directive.id) ? "bg-accent-primary-bg" : "bg-surface-card"].filter(Boolean).join(" ")} onClick={(e) => e.stopPropagation()}>
                    <Checkbox
