    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <Button
          ...
          className="inline-flex items-center gap-1.5 h-8 px-3 rounded text-sm font-medium bg-surface-card border border-accent-primary-bd text-accent-primary-fg hover:bg-surface-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <Button onClick={fetchDirectives} title="Refresh" className="inline-flex items-center justify-center h-8 w-8 rounded text-sm font-medium bg-surface-card border border-border-default text-foreground-default hover:bg-surface-hover transition-colors"><RefreshCw className="h-3.5 w-3.5" /></Button>
        <Button
      ...
      <div className="rounded-md bg-surface-card border border-on-dark/[0.08] flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-accent-primary-bg border-b border-accent-primary-bd shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-accent-secondary-fg bg-surface-inset border border-accent-secondary-bd px-2 py-0.5 rounded">{filteredDirectives.length} ({selected.size})</span>
          </div>
        </div>
        ...
        <table className="w-full caption-bottom text-sm">
          <thead>
            <tr className="border-b border-border-default">
              ...
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>ID</th>
              ...
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Statement</th>
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Active</th>
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Priority</th>
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Tags</th>
              <th className={["text-xs uppercase text-foreground-faint font-medium py-2 px-4 text-left", !freeze && "sticky top-0 z-20 bg-surface-card"].filter(Boolean).join(" ")}>Generated By</th>
          ...
          <tbody>
            {displayDirectives.length === 0 && !loading ? (
              <tr>
                <td colSpan={9} className="text-center py-8 text-foreground-faint">
              ...
              displayDirectives.map((directive) => (
                <tr
                  ...
                  className={`border-b border-border-subtle transition-colors cursor-pointer ${
                    selected.has(directive.id) ? 'bg-accent-primary-bg' : 'hover:bg-surface-card'
                  }`}
                  onClick={(e) => handleDirectiveClick(directive, e)}
                >
                  <td className={["py-1.5 px-4", freeze && "sticky left-0 z-10 border-r border-border-subtle", selected.has(directive.id) ? "bg-accent-primary-bg" : "bg-surface-card"].filter(Boolean).join(" ")} onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      checked={selected.has(directive.id)}
                      onCheckedChange={() => toggleSelection(directive.id)}
                    />
                  </td>
                  <td className="py-1.5 px-4 text-xs text-foreground-subtle font-mono">{directive.id}</td>
                  <td className="py-1.5 px-4 text-xs font-semibold text-foreground-default">
                    {directive.name ? (
                      <span className={search.trim() && directive.name.toLowerCase().includes(search.toLowerCase()) ? 'text-accent-primary-fg' : ''}>
                        {directive.name}
                      </span>
                    ) : (
                  ...
                  <td className="py-1.5 px-4 text-xs text-foreground-muted max-w-md">
                  ...
                  <td className="py-1.5 px-4 text-xs text-foreground-faint">{directive.priority ?? 0}</td>
                  ...
                  <td className="py-1.5 px-4 text-xs text-foreground-faint">{directive.generated_by}</td>
