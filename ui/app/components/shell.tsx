  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
            ...
            <div className={`flex items-center gap-2 ${collapsed ? 'justify-center' : ''}`}>
              ...
              {!collapsed && (
                <div className="flex items-center gap-2">
                  ...
                  {version ? (
                    <span className="text-[10px] text-foreground-subtle font-mono tabular-nums leading-tight whitespace-nowrap" title={`commit ${version.commit}`}>
                      v{version.version} · {version.commit}
                    </span>
                  ) : (
                    <span className="text-[10px] text-foreground-placeholder font-mono tabular-nums leading-tight whitespace-nowrap">
                      v… · …
                    </span>
                  )}
                </div>
        ...
        <header className="flex-1 h-[56px] bg-surface-panel px-6 flex items-center justify-end">
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              className="flex items-center gap-2 text-sm text-foreground-faint hover:text-foreground-default transition-colors"
            >
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* Sidebar Navigation */}
        <aside className={`border-r border-border-default bg-surface-panel flex flex-col transition-all duration-200 ${collapsed ? 'w-16' : 'w-64'}`}>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium transition-colors mx-1 ${
                    isActive 
                      ? 'bg-gradient-to-r from-accent-primary-bd to-accent-primary-fg text-foreground-default' 
                      : 'text-foreground-faint hover:text-foreground-default hover:bg-surface-panel'
          ...
          <div className="border-t border-border-default p-2">
            <button
              ...
              className={`flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium text-foreground-faint hover:text-foreground-default hover:bg-surface-panel transition-colors w-full mx-1 ${collapsed ? 'justify-center px-2' : ''}`}
            >
              {collapsed ? (
